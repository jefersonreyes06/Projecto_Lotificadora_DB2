...
BEGIN
    ...
    -- Sumar pagos por cuota y actualizar el saldo de la venta
    WITH PagosTotales AS (
        SELECT 
            pp.VentaID,
            SUM(i.MontoRecibido) AS TotalPagado
        FROM inserted i
    ...
    SET v.MontoFinanciado = CASE
        WHEN pt.TotalPagado >= v.MontoFinanciado THEN 0
        ELSE v.MontoFinanciado - pt.TotalPagado
    END
    FROM Ventas v
    INNER JOIN PagosTotales pt ON v.VentaID = pt.VentaID;
...
BEGIN
    ...
    INNER JOIN inserted i ON pp.CuotaID = i.CuotaID
    GROUP BY v.VentaID, v.MontoFinanciado
