...
BEGIN
    ...
    BEGIN TRY
        ...
        BEGIN
            ...
            INNER JOIN Etapas e ON b.EtapaID = e.EtapaID
            INNER JOIN Proyectos p ON e.ProyectoID = p.ProyectoID
            WHERE l.Estado IN ('Disponible', 'Proceso')
...
BEGIN
    ...
    DECLARE @FechaActual DATE = ISNULL(@FechaCierre, GETDATE());
    
    BEGIN TRY
        BEGIN TRANSACTION;
        
        -- Validar que el usuario existe
        IF @UsuarioCajaID IS NOT NULL AND NOT EXISTS (SELECT 1 FROM Usuarios WHERE UsuarioID = @UsuarioCajaID AND Rol = 'Cajero')
        BEGIN
        ...
        BEGIN
            ...
            INSERT INTO CierreCaja (UsuarioCajaID, FechaCierre, Turno, TotalEfectivoRecibido, TotalDepositadoBanco, TotalGeneral, CantidadPagosEfectivo, CantidadPagosDeposito, Estado)
            VALUES (@UsuarioCajaID, @FechaActual, @Turno, @TotalEfectivo, @TotalDeposito, @TotalGeneral, @CantidadEfectivo, @CantidadDeposito, 'Pendiente');
